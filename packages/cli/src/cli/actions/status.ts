...
export const status = (serviceName: string, version: string) => () => {
    ...
    try {
        if (isLinux) {
            ...
            try {
                const activeState = execSync(`systemctl show ${serviceName} --property=ActiveState`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('=')[1];
                const subState = execSync(`systemctl show ${serviceName} --property=SubState`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('=')[1];
                const mainPid = execSync(`systemctl show ${serviceName} --property=MainPID`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('=')[1];
                ...
                if (activeState === 'active') {
                    ...
                    try {
                        const memory = execSync(`systemctl show ${serviceName} --property=MemoryCurrent`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('=')[1];
                    ...
                    try {
                        const cpu = execSync(`ps -p ${mainPid} -o %cpu`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('\n')[1];
...
function statusWindowsService({ serviceName, mahameruInstalationPath, version }: { serviceName: string, mahameruInstalationPath: string; version: string }) {
    if (process.platform === 'win32') {
        try {
            ...
            if (existsSync(winswExePath)) {
                try {
                    const rawStatus = execSync(`"${winswExePath}" status`, { cwd: mahameruInstalationPath }).toString().trim();
            ...
            try {
                const scOutput = execSync(`sc query "${serviceName}"`, { stdio: ['pipe', 'pipe', 'ignore'] }).toString();
