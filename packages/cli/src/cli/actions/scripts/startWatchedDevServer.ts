...
export async function startWatchedDevServer({
  ...
  const watchman = new Watchman([DEV_DIST_DIR, configFilePath]);
    ...
    reloadTimer = setTimeout(() => {
      watcherQueue = watcherQueue
        .then(async () => {
          ...
          if (pendingRuntimePaths.length === 0) if (!shouldRunFullRuntimeRebuild) return;

          const changedFile = pendingRuntimePaths[pendingRuntimePaths.length - 1];
          const changedSourceFile =
            pendingSourceFilePaths[pendingSourceFilePaths.length - 1] ??
            (changedFile
              ? mapRuntimeFilePathToSourceFilePath(changedFile)
              : resolve(SOURCE_ROOT_DIR));
  ...
  watchman.on('all', ({ event, filePath }) => {
    ...
    if (kind === 'runtime' && event !== 'delete' && event !== 'rename') return;
...
async function startTypeScriptWatch({
  ...
  const reportWatchStatusChanged = (diagnostic: Diagnostic) => {
    ...
    if (isSuppressedTypeScriptWatchMessage(message)) return;

    console.log(`${pc.yellow('[Mahameru TSC]')} ${message}`);
  };

  const host: WatchCompilerHostOfConfigFile<EmitAndSemanticDiagnosticsBuilderProgram> =
    createWatchCompilerHost(
      DEV_TSCONFIG_PATH,
      {
  ...
  const hostWithWriteFile =
    host as WatchCompilerHostOfConfigFile<EmitAndSemanticDiagnosticsBuilderProgram> & {
      writeFile?: WriteFileCallback;
    };
  const originalWriteFile =
    hostWithWriteFile.writeFile?.bind(hostWithWriteFile) ?? sys.writeFile.bind(sys);
...
function classifyRuntimeArtifact(filePath: string): 'config' | 'runtime' | null {
  ...
  if (
    !normalizedFilePath.startsWith(`${normalizedDevRoot}/`) &&
...
function isRuntimeJavaScriptOutput(filePath: string) {
  ...
  return (
    filePath.startsWith(normalizedRuntimeRoot) &&
...
function collectRuntimeSourceFilePaths(sourceFiles?: readonly SourceFile[]) {
  ...
  return sourceFiles
    ...
    .filter((sourceFilePath) => {
      ...
      return (
        normalizedSourceFilePath.startsWith(normalizedSourceRoot) &&
...
function mapRuntimeFilePathToSourceFilePath(runtimeFilePath: string) {
  ...
  return resolve(SOURCE_ROOT_DIR, runtimeRelativePath.replace(/\.js$/i, '.ts'));
}
...
function mapSourceFilePathToRuntimeFilePath(sourceFilePath: string) {
  ...
  return resolve(DEV_RUNTIME_DIR, sourceRelativePath.replace(/\.ts$/i, '.js'));
}
