...
export class App {
  ...
  async start(): Promise<void> {
    if (this.initialized) throw new Error('App has already been started.');
  ...
  stop(): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (
        typeof this.options.gracefulShutdownTimeout !== 'undefined' &&
        this.options.gracefulShutdownTimeout > 0
      )
        timeoutId = setTimeout(() => {
          if (this.child) {
            console.warn(
              pc.yellow(`[Mahameru] Graceful shutdown timed out, killing child process.`),
            );
  ...
  protected buildAppEnv() {
    const env = {
      ...
      ...(this.options.multiCore
        ? { MAHAMERU__MULTI_CORE: this.options.multiCore.toString().trim() }
        : {}),
    };
