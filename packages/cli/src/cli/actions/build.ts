...
import { generateBarrelIndexFile, generateDataSourceTypes, generateMahameruDts, generateRouteTypes } from './scripts/generate-dynamic-types';
...
export function build({ rootPath }: { rootPath: string }) {
    return async () => {
        ...
        if (!productionDir)
            throw new Error('MAHAMERU__PRODUCTION_DIR is not set');
        ...
        try {
            ...
            await generateRouteTypes(join(rootPath, 'src', 'routes'), join(productionPath, 'types', 'routes.d.ts'));
            await generateDataSourceTypes(join(rootPath, 'src', 'databases'), join(productionPath, 'types', 'dataSources.d.ts'));
            ...
            if (unresolvedAliases.length > 0) {
                ...
                console.error(pc.yellow('Please use tsconfig path aliases such as @/* and avoid package.json imports for app source files.'));
            ...
            if (existsSync(defaultEnvFile))
                await copyFile(defaultEnvFile, join(productionPath, '.env'));
            ...
            await createZip(join(process.cwd(), productionDir), join(process.cwd(), `${productionDir}.zip`), false);
