...
export const install =
  ...
  ({
    ...
    cert,
    key,
  }: StrictServerOptions & { host: string; port: number; cert?: string; key?: string }) => {
    try {
      if (IS_LINUX) {
        ...
        try {
          appPath = execSync(`where mahameru`, { stdio: ['ignore', 'pipe', 'ignore'] })
            .toString()
            .trim();
...
async function onLinux({
  ...
  const workingPath = process.env.SUDO_HOME
    ? join(process.env.SUDO_HOME!, APPDATA_DIRNAME)
    : APPDATA_PATH;
  ...
  try {
    if (existsSync(servicePath))
      throw new Error(
        `Service "${serviceName}" is already registered on this system. Please run uninstall command first if you want to update it.`,
      );
...
`;
    ...
    console.log(
      `🌍 Mahameru Process Manager running at ${cert && key ? 'https' : 'http'}://${host}:${port}`,
    );
...
async function onWindows({
  ...
  try {
    ...
    console.log(
      `🌍 ${toTitleCase(serviceName)} running at ${cert && key ? 'https' : 'http'}://${host}:${port}`,
    );
