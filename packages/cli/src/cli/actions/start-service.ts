  ...
  try {
    if (isLinux) {
      ...
      if (!existsSync(servicePath)) {
        throw new Error(
          `Service "${serviceName}" is not registered on this system. Please run "mahameru pm service install" first.`,
        );
      }
      ...
      if (!isRunning) {
        ...
        console.log(
          `📝 To check service status, run: ${pc.bold(`systemctl status ${serviceName}`)}`,
        );
      } else {
...
function startWindowsService({ serviceName }: { serviceName: string }) {
  try {
    ...
    console.error(
      'Make sure you are running CMD/PowerShell as an Administrator and the service is installed.',
    );
  }
}
