...
export function relaunchCurrentProcessAsAdmin(workingDirectory: string) {
  ...
  if (!cliEntrypoint) return { ok: false as const, reason: 'missing-entrypoint' as const };
  ...
  const argumentList = ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-NoExit', '-Command', command];
  ...
  const result = spawnSync(
    'powershell.exe',
    ['-NoProfile', '-NonInteractive', '-Command', startProcessCommand],
    {
      encoding: 'utf8',
      windowsHide: true,
    },
