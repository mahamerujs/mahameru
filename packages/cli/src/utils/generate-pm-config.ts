...
export async function generatePmConfig({
  host,
  port,
  cert,
  key,
}: GeneratePmConfigOptions): Promise<ProcessManagerConfig> {
  ...
  if (cert && !(await exists(cert))) throw new Error('SSL Certificate file not found');

  if (key && !(await exists(key))) throw new Error('SSL key file not found');
