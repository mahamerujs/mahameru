...
import { ClientToServerEvents, InterServerEvents, ManagedProject, Payload, PayloadDelete, ProcessResponse, Project, ProjectNamePayload, ServerToClientEvents, SocketData } from "../types";
import { deleteProject as deleteProjectByName, deleteSocket, getProject, getProjects, getSockets, loadProjects, saveProjects, setProject, setProjects, setSocket } from "./";
...
export const processManager = async ({ host, port, daemon, cert, key, logins }: ProcessManagerConfig & { daemon: boolean; }, version: string) => {
    try {
        const adminLogin = logins.find(login => login.role === LoginRole.ADMIN);

        if (!adminLogin)
            throw new Error('Admin login not found');
        ...
        const handleHttpRequest = (request: IncomingMessage, response: ServerResponse<IncomingMessage> & { req: IncomingMessage; }) => {
            if (isDev) {
            ...
            if ((request.url === '/api/auth/login' || request.url === '/api/auth/login/') && request.method === 'POST') {
                let rawBody = '';
                ...
                request.on('end', () => {
                    try {
                        const body = JSON.parse(rawBody) as { username?: unknown; password?: unknown; } | undefined | null;
                        ...
                        const login = logins.find(login => login.username === body.username && login.password === body.password);
                        ...
                        response.setHeader('Set-Cookie', `mahameru_pm_token=${token}; Max-Age=${cookieMaxAge}; HttpOnly; Path=/;${isHttps ? ' Secure;' : ''} SameSite=Strict;`);
            ...
            if ((request.url === '/api/auth/session' || request.url === '/api/login/session') && request.method === 'POST') {
                ...
                const [username, password] = Buffer.from(cookies.mahameru_pm_token, 'base64').toString().split(':');

                const data = logins.find(login => login.username === username && login.password === password);

                if (!data) {
                    response.setHeader('Set-Cookie', `mahameru_pm_token=; Max-Age=0; HttpOnly; Path=/;${isHttps ? ' Secure;' : ''} SameSite=Strict;`);
            ...
            if ((request.url === '/api/auth/logout' || request.url === '/api/login/logout') && request.method === 'POST') {
                const rawCookies = request.headers.cookie;
        ...
        const httpServer = cert && key ? createHttpsServer({
        ...
        const handleStdout = (projectName: string) =>
            (data: any) => {
        ...
        const handleMessage = (socket: net.Socket | null, projectName: string, child: ChildProcess) => ({ type, data }: MahameruIPCMessageServer) => {
            if (!socket)
                return
            ...
            if (type === 'ERROR') {
                ...
                console.log(`[MPM] Project${projectName} successfully started by PM Daemon (PID: ${child.pid})`);
                ...
                if (project)
                    socketServer.emit('process-usage', { data, project: parseProject(project) })
            }
        }
        ...
        const getRunnerScript = (project: Pick<ManagedProject, 'rootPath' | 'entryFilePath' | 'packageJson'>) => {
        ...
        const attachChildProcessListeners = async (project: ManagedProject, child: ChildProcess, socket: net.Socket | null = null) => {
            const logFilePath = join(project.rootPath, 'logs', 'out.log');
            const errorFilePath = join(project.rootPath, 'logs', 'error.log');

            if (!existsSync(project.logDirPath))
                await mkdir(project.logDirPath, { recursive: true });

            if (!existsSync(logFilePath))
                await writeFile(logFilePath, '', 'utf-8');

            if (!existsSync(errorFilePath))
                await writeFile(errorFilePath, '', 'utf-8');

            if (process.env.SUDO_UID && process.env.SUDO_GID)
                await chownR(project.logDirPath, parseInt(process.env.SUDO_UID), parseInt(process.env.SUDO_GID));
        ...
        const waitForChildMessage = <T extends MahameruIPCMessageServer['type']>(
            ...
            new Promise<Extract<MahameruIPCMessageServer, { type: T }>>((resolve) => {
                const handleChildMessage = (message: MahameruIPCMessageServer) => {
                    if (!expectedTypes.includes(message.type as T))
                        return;
                ...
                try {
                    ...
                    child.once('exit', (code) => {
                        settle(() => {
                            ...
                            reject(new Error(`Project ${project.name} exited before ready${typeof code === 'number' ? ` with code ${code}` : ''}`));
                        });
                    });
        ...
        const deleteProject = async (projectName: string): Promise<Project> => {
            ...
            logDeleteAudit(`Project "${projectName}" exists after delete: ${getProject(projectName) ? 'true' : 'false'}`);
            ...
            socket.on('data', (data) => {
                try {
                    ...
                    if (message.command === 'START') {
                        ...
                        if (project && project.status === 'running' && typeof project.child !== 'undefined' && typeof project.pid !== 'undefined') {
                        ...
                        if (!project) {
                            socket.write(JSON.stringify({ success: false, error: `Project ${payload.packageJson.name} not found` }));
        ...
        socketServer.on('connection', (socket) => {
            ...
            socket.on('start', (name, callback) => {
                ...
                if (!project)
                    return callback(toError('Project not found'));
            ...
            const handleDelete = (eventName: 'delete' | 'remove') => (name: string, callback: (response: ProcessResponse<Project>) => void) => {
