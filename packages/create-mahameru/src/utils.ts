...
export async function readPackageJsonFile(rootPath?: string): Promise<PackageJson> {
    ...
    try {
        if (!existsSync(path))
            throw new Error(`package.json file not found at ${path}`);
...
export async function writePackageJsonFile(packageJson: PackageJson, targetPath?: string) {
    const path = targetPath ? (!targetPath.endsWith('.json') ? `${targetPath}.json` : targetPath) : join(process.cwd(), 'package.json');
...
export async function getGithubPublicRepo(username: string) {
    ...
    repos = repos.filter((repo: any) => repo.private === false && repo.name.startsWith('mahameru-template-'))
...
export async function getPackageJsonFromGithubResponse(repo: FullRepo): Promise<PackageJson> {
    const responsePackageJson = await fetch(`https://raw.githubusercontent.com/${repo.full_name}/refs/heads/main/package.json`, {
        method: 'GET',
        headers: {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36'
        }
