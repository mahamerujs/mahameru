...
const start = async (rootPath: string, host: string, port: number) => {
  ...
  let mahameruConfig = await runTypescriptFile<MahameruConfigResult>(mahameruConfigFilePath, {
    cleanup: true,
  });
  ...
  if (mahameruConfig.partial?.host) host = mahameruConfig.partial.host;

  if (mahameruConfig.partial?.port) port = mahameruConfig.partial.port;

  if (!mahameruConfig.partial?.host) mahameruConfig.merged.host = host;

  if (!mahameruConfig.partial?.port) mahameruConfig.merged.port = port;
...
(async () => {
  try {
    ...
    if (!host) throw new Error('MAHAMERU__HTTP_LISTEN_HOST environment variable is not set.');

    if (!port) throw new Error('MAHAMERU__HTTP_LISTEN_PORT environment variable is not set.');
    ...
    const handleProcessOnMessage = async (message: DevServerParentProcessMessage) => {
      ...
      if (message.type === 'START') {
        ...
        if (app && app.initialized) await app.devHRM(message.filePath);
      }
    };
