...
export class Generator {
    ...
    async start() {
        const routesIndexFilePath = join(this.options.sourceDirPath, this.options.routesDir, 'route.ts');
        const moduleSourcePath = join(this.options.sourceDirPath, this.options.modulesDir);

        if (!existsSync(moduleSourcePath))
...
`;
    ...
    public async generateInstancesDTS(): Promise<void> {
        try {
            ...
            const importStatements = [...imports.entries()].map(([specifier, names]) =>
