...
export class Route {
    ...
    async runNotFoundHandler(request: MagmaRequest, method: HTTPMethod): Promise<MagmaResponse | undefined> {
    ...
    async resolveRoute(request: MagmaRequest) {
        ...
        if (!matchedRoute || !matchResult)
            return {
                matchedRoute: null,
                matchResult: null,
                notFoundResponse: await this.runNotFoundHandler(request, request.method) || MagmaResponse.json(
                    { error: 'Not Found' },
                    { status: 404 }
                )
            };
    ...
    protected isMagmaResponseLike(value: unknown): value is { body: unknown; status: number; headers?: Headers | Record<string, string> } {
