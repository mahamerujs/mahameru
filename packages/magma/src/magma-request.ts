...
export class MagmaRequest {
  ...
  public get userAgent(): ParsedUserAgent {
    ...
    if (ua.includes('windows')) {
      ...
      os =
        ua.includes('ipad') || (ua.includes('macintosh') && 'maxTouchPoints' in globalThis)
          ? 'iOS'
          : 'macOS';
  ...
  public async files(options?: {
    ...
    while (index < rawBody.length) {
      ...
      if (rawBody.indexOf(endBoundaryBuffer, boundaryIndex) === boundaryIndex) break;

      let nextBoundaryIndex = rawBody.indexOf(
        boundaryBuffer,
        boundaryIndex + boundaryBuffer.length,
      );

      if (nextBoundaryIndex === -1)
        nextBoundaryIndex = rawBody.indexOf(
          endBoundaryBuffer,
          boundaryIndex + boundaryBuffer.length,
        );
      ...
      const partBuffer = rawBody.subarray(
