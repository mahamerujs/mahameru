...
export class MahameruDevServer extends EventEmitter {
    ...
    public override emit<K extends keyof MahameruDevServerEvents>(event: K, ...args: MahameruDevServerEvents[K]): boolean {
        return super.emit(event, ...args);
    }

    public override on<K extends keyof MahameruDevServerEvents>(event: K, listener: (...args: MahameruDevServerEvents[K]) => void): this {
        return super.on(event, listener as any);
    }

    public override once<K extends keyof MahameruDevServerEvents>(event: K, listener: (...args: MahameruDevServerEvents[K]) => void): this {
    ...
    public async build(): Promise<boolean> {
        try {
            ...
            const compilerOptionsLookup = ts.createModuleResolutionCache(this.options.rootPath, host.getCanonicalFileName, options);

            host.resolveModuleNameLiterals = (moduleLiterals, containingFile, redirectedReference, options) => {
            ...
            if (errorReports.length > 0)
                process.send?.({ type: 'ERROR', data: errorReports });

            if (!emitResult.emitSkipped)
                if (await this.buildMahameruConfig())
                    await this.tscAlias();
    ...
    public async tscAlias() {
        ...
        try {
            ...
            await writeFile(tsconfigBuildFilePath, JSON.stringify({
                ...tsconfig,
                compilerOptions: {
                    ...(tsconfig.compilerOptions || {}),
