...
export class Mahameru {
            ...
            if (!onMessage) {
                onMessage = (message) => {
                    if (message["compile-error"]) {
                        ...
                        this.typescriptServer.errors = message["compile-error"][0].length > 0 ? message["compile-error"][0].map(m => m.formatted).join('\n\n') : undefined;

                        if (this.typescriptServer.status !== 'READY')
                            return;
    ...
    public static generator({ dev, rootPath, outputTypesDirPath }: MahameruGeneratorOptions, logger: Logger) {
        return {
            mahameruDts: async () => {
            ...
            env: async () => {
                {
                    ...
                    if (!existsSync(outputTypesDirPath))
                        await mkdir(outputTypesDirPath, { recursive: true });
    ...
    protected async discoverPlugins() {
        try {
            ...
            const allDependencies = { ...consumerPackagejson.dependencies, ...consumerPackagejson.devDependencies };

            const potentialPluginNames = Object.keys(allDependencies).filter(dep =>
                dep.startsWith('@mahameru/') && dep !== '@mahameru/diatrema'
            );

            this.logger.debug(`Found ${potentialPluginNames.length > 1 ? `${potentialPluginNames.length} plugins` : '1 plugin'}:`, potentialPluginNames.join(', '));

            for (const name of potentialPluginNames) {
                try {
                    const pluginDirPath = join(this.options.rootPath, 'node_modules', name);
                    const pluginPkg = JSON.parse(await readFile(join(pluginDirPath, 'package.json'), 'utf-8'));

                    if (pluginPkg?.mahameru?.type !== 'plugin')
                        continue;
                    ...
                    if (pluginInstance.generator) {
                        const pluginOutputTypesDirPath = join(this._options.outputTypesDirPath, shortPluginName);
    ...
    protected loadEnvironmentVariables() {
        ...
        if (Object.keys(envForProcess).length === 0)
            return;
