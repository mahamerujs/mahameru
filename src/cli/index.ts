...
(async () => {
    try {
        ...
        mahameru.command('start')
            ...
            .option('-H, --host <string>', 'Host to run the server on', '127.0.0.1')
            .option('--multi-core <number>', 'Enable multi-core support. For example: --multi-core 4 to use 4 cores or --multi-core -1 to use all cores')
        ...
        const mahameruPm = mahameru.command('pm').description(`${pc.bold(pc.cyan('▲ MahameruJS'))} ${pc.dim(`Process Manager`)}`);

        mahameruPm.command('list')
            .alias('ls')
            .description('List all registered projects.')
            .action(projectList(rootPath, version));

        mahameruPm.command('start')
        ...
        const mahameruPmService = mahameruPm.command('service')
            .description(`${pc.bold(pc.cyan('▲ MahameruJS'))} ${pc.dim(`Process Manager Service`)}`);

        mahameruPmService.command('install')
            .description('Install MahameruJS Process Manager and register the server as a service. This will make MahameruJS Process Manager boot on system startup.')
            .option('-p, --port <number>', 'Port to run the server on', parsePort, 8000)
            .option('-H, --host <string>', 'Host to run the server on', '127.0.0.1')
            .option('--cert <string>', 'Path to the SSL certificate file')
            .option('--key <string>', 'Path to the SSL key file')
        ...
        mahameruPmService.command('uninstall')
            .description('Uninstall MahameruJS Process Manager as a service. This will remove the service from systemd and disable it.')
            .action(uninstall(serviceName));

        mahameruPmService.command('start')
            .description('Start MahameruJS Process Manager service.')
            .action(startService(serviceName));

        mahameruPmService.command('stop')
        ...
        mahameruPmService.command('status')
            .description('Get MahameruJS Process Manager status.')
