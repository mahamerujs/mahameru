...
import { clearScreen, getGithubPublicRepo, readPackageJsonFile, writePackageJsonFile } from './utils';
...
(async () => {
    try {
        ...
        program
            ...
            .description('Initialize a new project Mahameru - A minimal and fast Node.js framework for building HTTP servers')
            .action(onInit);

        program.parseAsync(process.argv);
    } catch (error) {
        if (error instanceof Error && (error.name === 'ExitPromptError' || error.message.includes('force closed'))) {
...
async function installProjectDependencies(targetDir: string, dependencies?: Partial<Record<string, string>> | undefined, devDependencies?: Partial<Record<string, string>> | undefined) {
    if (!dependencies && !devDependencies) {
        return true;
    }

    const npmRunner = getNpmRunner();
    const depsToInstall = Object.entries(dependencies ?? {}).map(([pkg, version]) => `${pkg}@${version}`);
    const devDepsToInstall = Object.entries(devDependencies ?? {}).map(([pkg, version]) => `${pkg}@${version}`);

    try {
        if (depsToInstall.length > 0) {
            for (const dep of depsToInstall) {
                const rawTitle = dep.startsWith('@') ? dep.substring(1).replace('^', '') : dep.replace('^', '');
        ...
        if (devDepsToInstall.length > 0) {
            for (const devDev of devDepsToInstall) {
                const rawTitle = devDev.startsWith('@') ? devDev.substring(1).replace('^', '') : devDev.replace('^', '');
                const depTitle = rawTitle.split('@')[0];
                const depVersion = rawTitle.split('@')[1].replace(/^/g, '');
                const installSpinner = ora(`Installing ${depTitle} v${depVersion}...`).start();
...
async function ensureGlobalCliInstalled() {
    ...
    try {
        ...
        if (process.platform === 'win32') {
            console.log('Please install the CLI manually from Command Prompt / PowerShell as Administrator:');
        } else {
...
async function onInit() {
    ...
    if (selectedTemplate.packageJson.dependencies && Object.keys(selectedTemplate.packageJson.dependencies).length > 0)
        console.log(pc.cyan(`\nInstalling dependencies:\n   ${Object.keys(selectedTemplate.packageJson.dependencies).join('\n   ')}`));

    if (selectedTemplate.packageJson.devDependencies && Object.keys(selectedTemplate.packageJson.devDependencies).length > 0)
        console.log(pc.cyan(`\nInstalling dev dependencies:\n   ${Object.keys(selectedTemplate.packageJson.devDependencies).join('\n   ')}\n`));

    await installProjectDependencies(targetDir, selectedTemplate.packageJson.dependencies, selectedTemplate.packageJson.devDependencies);
    ...
    console.log(`Have a question? Join the discord server at ${pc.cyan('https://discord.gg/7PNmMxykSF')}\n`);
}
