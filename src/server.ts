...
(async () => {
    try {
        ...
        process.on('message', handleParentMessage(app));
...
function handleParentMessage(app: Mahameru | null) {
    return async (message: MahameruIPCMessageChild) => {
        ...
        switch (message.type) {
            ...
            case 'RELOAD':
                console.log('Reloading runtime state...');

                await app.reloadRuntimeState();

                console.log(`Reloading runtime state... Done`);
            ...
            case 'RESTART':
                ...
                console.log(`Restarting server... Done`);
