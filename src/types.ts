...
export type ErrorHandler = (context: MagmaContext, error: unknown, next: MagmaNext) =>
    Promise<MagmaResponse> | MagmaResponse;

export type RouteHandler = (context: MagmaContext) =>
    Promise<MagmaResponse> | MagmaResponse;
...
export type ContainerItem =
    | { name: string; path: string; type: 'module-service'; isPublic: boolean; item: unknown; moduleMeta?: ModuleMeta }
    | { name: string; path: string; type: 'module-controller'; isPublic: boolean; item: unknown; moduleMeta?: ModuleMeta }
    ...
    | { name: string; path: string; type: 'not-found'; isPublic: boolean; item: Partial<Record<HTTPMethod, RouteHandler>> }
    | { name: string; path: string; type: 'error-handler'; isPublic: boolean; item: ErrorHandler }
    | { name: string; path: string; type: 'protected-route'; isPublic: boolean; item: ProtectedRoute };
